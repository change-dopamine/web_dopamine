$(function () {
    ...
    var q = {
        ...
        pagesize: 10, // 每页显示几条数据,默认每页显示 12 条
        cate_id: '', // 文章分类的 Id
    ...
    function renderPage(total) {
        ...
        laypage.render({
            ...
            limits: [10, 20, 30, 50],
            // 分页发生切换的时候吗，触发 jump 回调
            // 触发 jump 方法的方式又两种
            // 1. 点击页码的时候会触发 jump 回调
            // 2. 只要调用了 laypage.render() 方法就会触发 jump 会回调
            jump: function (obj, first) {
                ...
                // 把罪行的页码值，赋值给 q 这个查询对象中
                q.pagenum = obj.curr

                // 把最新的条目数，赋值到 q 这个查询对象的 pagesize 属性中
                q.pagesize = obj.limit

                // 根据最新的 q 获取对应数据列表，并渲染表格
                if (!first) {
                    initTable()
                }
            }
        ...
        layer.confirm('确定删除?', { icon: 3, title: '提示' }, function (index) {
            $.ajax({
                ...
                success: function (res) {
                    ...
                    // 当数据完成删除后，需要判断当前这一页中，是否还有剩余的数据，如果没有剩余的数据了，则让页码值减一，之后再调用 initTable 方法
                    if (len === 1) {
